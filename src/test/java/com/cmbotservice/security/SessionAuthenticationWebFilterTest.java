 ...
 * stub {@link SessionStore}, same "no Spring context needed" style already used by {@code
 * ChatOrchestrationServiceTest}. Covers every rejection path the documented flow specifies, plus
 * the success path and both Redis-failure modes (reject by default, fail-open when configured).
 */
class SessionAuthenticationWebFilterTest {
  ...
  private static SecurityProperties properties(
      boolean csrfEnabled, boolean permitWhenNoPermissions, boolean failOpen) {
  ...
  private static SessionContext validSession() {
    return new SessionContext(
        "alice",
        "tenant-1",
        List.of("CHATBOT_CHAT", "OTHER_PERM"),
        List.of("org-1"),
  ...
  void nonChatPath_bypassesTheFilterEntirely_evenWithNoSessionCookie() {
    SessionAuthenticationWebFilter filter =
        new SessionAuthenticationWebFilter(
            cookieValue ->
                Mono.error(
                    new AssertionError("SessionStore must not be called for a non-chat path")),
  ...
  void missingCookie_rejectsWith401Unauthenticated() {
    SessionAuthenticationWebFilter filter =
        new SessionAuthenticationWebFilter(
            request -> Mono.error(new AssertionError("SessionStore must not be called")),
            properties(true, false, false),
            objectMapper);
  ...
  void csrfHeaderMissing_rejectsWith403Forbidden() {
    ...
    MockServerWebExchange exchange =
        MockServerWebExchange.from(
            MockServerHttpRequest.post("/api/v1/chat/messages")
                .cookie(
                    new HttpCookie(COOKIE_NAME, "abc123"),
                    new HttpCookie(CSRF_COOKIE_NAME, "csrf-value"))
                .build());
  ...
  void csrfDisabled_skipsCsrfCheckEntirely() {
    SessionAuthenticationWebFilter filter =
        new SessionAuthenticationWebFilter(
            cookieValue -> Mono.just(validSession()),
            properties(false, false, false),
            objectMapper);
    MockServerWebExchange exchange =
        exchangeWithSessionCookie("abc123"); // no CSRF cookie/header at all
    AtomicBoolean chainInvoked = new AtomicBoolean();
  ...
  void tenantHeaderAbsent_isNotValidatedAndRequestSucceeds() {
    SessionAuthenticationWebFilter filter =
        new SessionAuthenticationWebFilter(
            ...
            objectMapper);
    MockServerWebExchange exchange =
        exchangeWithSessionCookie("abc123"); // no tenant header sent at all
    AtomicBoolean chainInvoked = new AtomicBoolean();
  ...
  void noChatbotPermissions_rejectsWith403Forbidden_whenNotPermissive() {
    ...
    SessionAuthenticationWebFilter filter =
        new SessionAuthenticationWebFilter(
            cookieValue -> Mono.just(noChatbotPerms),
            properties(false, false, false),
            objectMapper);
    MockServerWebExchange exchange = exchangeWithSessionCookie("abc123");
  ...
  void validSession_passesThroughAndPopulatesExchangeAttributes() {
    ...
    List<String> grantedAuthorities =
        exchange.getAttribute(SessionAuthenticationWebFilter.GRANTED_AUTHORITIES_ATTRIBUTE);
  ...
  void redisUnavailable_rejectsWith503_byDefault() {
    SessionAuthenticationWebFilter filter =
        new SessionAuthenticationWebFilter(
            cookieValue ->
                Mono.error(new io.lettuce.core.RedisConnectionException("connection refused")),
            properties(false, false, false),
            objectMapper);
    MockServerWebExchange exchange = exchangeWithSessionCookie("abc123");
    AtomicBoolean chainInvoked = new AtomicBoolean();
