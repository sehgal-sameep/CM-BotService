...
/**
 * Verifies {@link JsonBlobSessionStore}'s parsing of the assumed session JSON shape
 * against a mocked {@link ReactiveStringRedisTemplate} — no real Redis needed. See the
 * class's own Javadoc for the caveat this default strategy's layout is unconfirmed;
 * these tests document exactly what it currently assumes.
 */
...
class JsonBlobSessionStoreTest {
    ...
    void sessionFound_parsesAllConfiguredFieldsCorrectly() {
        ...
        StepVerifier.create(store.findSession("abc123"))
                .expectNextMatches(ctx -> ctx.username().equals("alice")
    ...
    void expiryAsEpochMillisNumber_isAlsoAccepted() {
        ...
        String json = "{\"username\":\"alice\",\"tenantId\":\"tenant-1\",\"permissions\":[],\"organizations\":[],"
                + "\"accessTokenExpiry\":" + epochMillis + "}";
        when(valueOperations.get("ns:abc123")).thenReturn(Mono.just(json));
    ...
    void missingOptionalFields_areToleratedAsNullOrEmpty() {
        String json = "{\"username\":\"alice\",\"tenantId\":\"tenant-1\",\"accessTokenExpiry\":\"2030-01-01T00:00:00Z\"}";
        when(valueOperations.get("ns:abc123")).thenReturn(Mono.just(json));

        StepVerifier.create(store.findSession("abc123"))
                .expectNextMatches(ctx -> ctx.permissions().isEmpty()
    ...
    void unparsableExpiry_isTreatedAsNotFoundRatherThanPropagatingAnError() {
        String json = "{\"username\":\"alice\",\"tenantId\":\"tenant-1\",\"accessTokenExpiry\":\"not-a-date\"}";
    ...
    void redisConnectionFailure_propagatesAsAnErrorRatherThanEmpty() {
        when(valueOperations.get(anyString())).thenReturn(Mono.error(new RedisConnectionException("connection refused")));
