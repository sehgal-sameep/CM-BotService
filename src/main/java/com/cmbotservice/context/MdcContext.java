...
/**
 * MDC key constants, plus the one-time startup wiring that makes those keys survive
 * across Reactor's thread hops.
 * <p>
 * In a reactive pipeline, MDC (thread-local) does not automatically follow a value
 * written into a Reactor {@link reactor.util.context.Context} — that's exactly what
 * Micrometer's {@link ContextRegistry}/{@link ThreadLocalAccessor} SPI exists to
 * bridge (it's the same mechanism that makes {@code traceId}/{@code spanId} show up in
 * logs from a reactive app). Registering our four keys here means anything upstream
 * that does {@code .contextWrite(ctx -> ctx.put(MdcContext.CORRELATION_ID, id))} (see
 * {@link CorrelationIdFilter}, {@code ChatOrchestrationService}) gets that value
 * restored into MDC automatically at every subsequent operator boundary — request-
 * scoped, not a global/shared {@code ThreadLocal}, and impossible to leak into another
 * request's logs, since Reactor {@code Context} is immutable and per-subscription.
 */
@Component
