 ...
 * {@code caseId}/{@code history} are just forwarded, not validated against any stored
 * record. {@code tenantId}/{@code organization} are <b>not</b> here — the frontend
 * sends those as the {@code X-Tenant-Id}/{@code X-Org-Id} request headers
 * instead (see {@code RequestContextResolver}), not the body.
 */
public record ChatRequest(
        ...
        @Schema(
                description = "Explicit conversation transcript, oldest turn first. The ML Agent's contract has "
        ...
        @Schema(
                description = "Optional hint forwarded as-is to the ML Agent's case context. Exact "
