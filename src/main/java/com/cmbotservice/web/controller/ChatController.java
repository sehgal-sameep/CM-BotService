...
public class ChatController {
    ...
    @Operation(
            ...
            description = """
                    ...
                    `tenantId`, `caseId`, and `message` are required. `conversationId` is optional: \
                    omit it to start a new conversation (the ML Agent assigns one) or send back the \
                    id from a previous `stream-start` event to continue one. `requestId` is an \
                    optional caller-generated identifier forwarded for tracing/correlation only \
                    (idempotency-friendly, not deduplicated anywhere). The caller — not this \
                    backend — is responsible for remembering and resending the conversation id.

                    SSE event contract (in order):
                    - `stream-start` — { conversationId, messageId, timestamp } — conversationId here \
                      is the one resolved by the ML Agent (echoed or newly assigned); capture it for \
                      the next request.
                    - `message` (zero or more) — { conversationId, messageId, sequence, content, timestamp }
                    - exactly one of:
                      - `stream-complete` — { conversationId, messageId, totalChunks, timestamp }
                    ...
                    CONCURRENCY_LIMIT_REACHED (bulkhead full or circuit breaker open), INTERNAL_ERROR. \
                    Because the HTTP status is already committed to 200 by the time any of these can \
                    occur, every post-acceptance failure — including a rejected/timed-out ML Agent \
                    call — surfaces as an `error` event on this same stream, never a different HTTP \
                    status.
                    ...
                    curl -N -X POST "http://localhost:8080/api/v1/chat/messages" \\
                      ...
                      -d '{"tenantId":"tenant-123","caseId":"case-456","message":"Summarize this case for me"}'
                    ```
                    """
    )
    @ApiResponse(responseCode = "200", description = "SSE stream of chatbot events",
            content = @Content(mediaType = MediaType.TEXT_EVENT_STREAM_VALUE))
    @ApiResponse(responseCode = "400", description = "Validation error (e.g. blank tenantId/caseId/message)",
    ...
    public Flux<ServerSentEvent<Object>> sendMessage(@RequestBody @Valid ChatRequest request, ServerWebExchange exchange) {
        RequestContext context = requestContextResolver.resolve(exchange, request.tenantId(), request.caseId());
