...
/**
 * Converts a {@link ChatSseEvent} into the {@code ServerSentEvent} Spring actually writes to the
 * response. A pure mapping function — no state, no I/O — unlike the imperative {@code
 * SseStreamWriter} this replaces; sequencing across event types is done once, upstream, via {@code
 * Flux#index()} in {@code ChatOrchestrationService}, and handed in here as {@code frameId} rather
 * than tracked with a mutable counter.
 */
