...
/**
 * Glue between the web layer and the {@link MlAgentClient}: forwards one message,
 * calls the ML Agent through circuit breaker + bulkhead + timeout + retry, and relays
 * the streamed response as SSE. Fully non-blocking end to end — nothing here holds a
 * thread, sleeps, or buffers the full response; every layer of protection is a
 * declarative {@code Flux} operator, and cancellation (a client disconnect) tears the
 * whole chain down automatically via Reactor's own propagation.
 * <p>
