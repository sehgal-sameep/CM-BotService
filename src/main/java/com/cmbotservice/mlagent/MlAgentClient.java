...
public interface MlAgentClient {
  ...
  /**
   * Streams a response for {@code request} as a cold, backpressure-respecting {@code Flux}: nothing
   * happens until subscribed, and cancelling the subscription (a client disconnect propagates here
   * automatically) stops any further work. Timeout, retry, circuit-breaker, and bulkhead behavior
   * are applied uniformly to whatever this returns by {@code ChatOrchestrationService} —
