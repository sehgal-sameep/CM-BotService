 ...
 * <p>
 * Mirrors the real contract's shape: a structured {@link MlAgentStreamEvent.Payload}
 * (with a satisfying citation on every key signal) is always sent before
 * {@link MlAgentStreamEvent.Done}, so the SSE path is fully exercisable without a real
 * agent.
 * <p>
...
public class MockMlAgentClient implements MlAgentClient {
    ...
    private Flux<MlAgentStreamEvent> streamChunks(String text, Duration delay) {
        ...
        return Flux.concat(
                ...
                Mono.just(new MlAgentStreamEvent.Done(approxLatencyMs, approxTokensIn, approxTokensOut, false)));
    }

    /**
     * Fabricates a structured payload that satisfies the one invariant
     * {@link CaseSummaryPayloadValidator} enforces on a real response — every key
     * signal carries a citation — so the mock exercises the exact same downstream path
    ...
    private static CaseSummaryPayload buildPayload() {
        ...
        CaseSummaryPayload.KeySignal signal = new CaseSummaryPayload.KeySignal(
                "Transaction deviates from the customer's typical behavior profile", List.of(citationId));
