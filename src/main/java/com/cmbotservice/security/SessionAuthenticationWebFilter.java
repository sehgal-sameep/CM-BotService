...
/**
 * Enforces the documented BFF-session authentication flow: extract session cookie →
 * look up in the shared Redis store (read-only) → check expiry → validate CSRF →
 * cross-check tenant → filter/authorize by {@code CHATBOT_}-prefixed permission. On
 * success, stores a {@link SessionContext} (and the filtered granted-authorities list)
 * as exchange attributes for {@link SessionRequestContextResolver} to consume; on
 * rejection, writes this service's own {@link ErrorResponse} JSON shape directly.
 * <p>
 * Runs only when {@code chatbot.security.mode: BFF_SESSION} — see
 * {@link ChatbotSecurityMode} for the master toggle. Ordered right after
 * {@code CorrelationIdFilter} so a correlation ID is always available for both the
 * response header and rejection logging. Only actually gates
 * {@link ApiPaths#CHAT_MESSAGES} — every other path (actuator health/readiness,
 * Swagger UI, OpenAPI JSON) passes through untouched, since those have their own
 * unauthenticated consumers (a k8s liveness/readiness prober has no BFF session
 * cookie to send).
 * <p>
 * <b>Why rejections are written directly instead of thrown:</b> a {@code WebFilter}
 * runs upstream of {@code DispatcherHandler}, so an exception thrown here never
 * reaches {@code @RestControllerAdvice} (that only intercepts exceptions raised during
 * controller/handler dispatch) — it would instead fall through to Spring Boot's
 * generic default error page, a different JSON shape than the {@link ErrorResponse}
 * contract every other error path in this service already uses. Writing the body here
 * keeps that contract uniform.
 */
@Component
...
public class SessionAuthenticationWebFilter implements WebFilter {
    ...
    public SessionAuthenticationWebFilter(SessionStore sessionStore, SecurityProperties properties,
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        ...
        HttpCookie sessionCookie = exchange.getRequest().getCookies().getFirst(properties.session().cookieName());
        ...
        return sessionStore.findSession(sessionCookie.getValue())
                .map(Optional::of)
                ...
                .onErrorResume(SessionAuthenticationWebFilter::isRedisFailure,
    ...
    private Mono<Void> continueWithSession(ServerWebExchange exchange, WebFilterChain chain, SessionContext session) {
        ...
        if (grantedAuthorities.isEmpty() && !properties.authorization().permitWhenNoChatbotPermissions()) {
            return reject(exchange, AuthRejectionReason.NO_CHATBOT_PERMISSIONS, session);
        }

        log.debug("AUTH_SUCCEEDED username={} tenantId={} grantedAuthorityCount={}",
                session.username(), session.tenantId(), grantedAuthorities.size());
    ...
    private boolean csrfValid(ServerWebExchange exchange) {
        HttpCookie csrfCookie = exchange.getRequest().getCookies().getFirst(properties.csrf().cookieName());
        String csrfHeader = exchange.getRequest().getHeaders().getFirst(properties.csrf().headerName());
        return csrfCookie != null && StringUtils.hasText(csrfCookie.getValue())
                && StringUtils.hasText(csrfHeader)
                && csrfCookie.getValue().equals(csrfHeader);
    }

    /**
     * The tenant header is an optional cross-check, not a required one: the flow only
     * validates it when the caller actually sends it — absent entirely is not a
     * mismatch.
     */
    private boolean tenantValid(ServerWebExchange exchange, SessionContext session) {
        String tenantHeaderValue = exchange.getRequest().getHeaders().getFirst(properties.session().tenantHeaderName());
    ...
    private Mono<Void> handleStoreError(ServerWebExchange exchange, WebFilterChain chain, Throwable ex) {
        if (properties.failOpenOnRedisError()) {
            log.warn("AUTH_FAIL_OPEN reason={} — chatbot.security.fail-open-on-redis-error is true; permitting this "
                            + "request WITHOUT a validated session. This must never be enabled in a shared/prod environment. cause={}",
                    AuthRejectionReason.SESSION_STORE_UNAVAILABLE.logToken(), ex.toString());
            return chain.filter(exchange);
        }
        log.error("AUTH_REJECTED reason={} cause={}", AuthRejectionReason.SESSION_STORE_UNAVAILABLE.logToken(), ex.toString());
        return writeRejection(exchange, AuthRejectionReason.SESSION_STORE_UNAVAILABLE);
    }

    private Mono<Void> reject(ServerWebExchange exchange, AuthRejectionReason reason, SessionContext session) {
        logRejection(exchange, reason, session);
        return writeRejection(exchange, reason);
    }

    private void logRejection(ServerWebExchange exchange, AuthRejectionReason reason, SessionContext session) {
        ...
        if (session != null) {
            log.warn("AUTH_REJECTED reason={} cookiePresent={} cookieLength={} username={} tenantId={}",
                    reason.logToken(), present, length, session.username(), session.tenantId());
        } else {
            log.warn("AUTH_REJECTED reason={} cookiePresent={} cookieLength={}", reason.logToken(), present, length);
        }
    }
    ...
    private Mono<Void> writeRejection(ServerWebExchange exchange, AuthRejectionReason reason) {
        ...
        ErrorResponse body = ErrorResponse.of(reason.errorCode(), rejectionMessage(reason), correlationId);
    ...
    private static String rejectionMessage(AuthRejectionReason reason) {
        return switch (reason) {
            ...
            case SESSION_STORE_UNAVAILABLE -> "Authentication service is temporarily unavailable; please try again shortly.";
        };
    }
}
